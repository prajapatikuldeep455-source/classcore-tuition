...
async function saveAndGenerateReportPDF(){
  ...
  const bodyHtml  = _buildReportCardBody(e, s, subjects, false);
  const html      = _wrapReportCardHTML(bodyHtml, filename);
...
function _buildReportCardBody(e, s, subjects, isPageInBatch = false) {
  const inst      = localStorage.getItem('tuitionName')||'ClassCore Tuition';
  const addr      = localStorage.getItem('tuitionAddress')||'';
  const logo      = localStorage.getItem('tuitionLogo')||'';
  const totalMax  = subjects.reduce((a,sub)=>a+(sub.max||0),0);
  const totalObt  = subjects.reduce((a,sub)=>a+(sub.obtained!==''?+sub.obtained:0),0);
  const pct       = totalMax>0 ? Math.round((totalObt/totalMax)*100) : 0;
  const grade     = pct>=90?'A+':pct>=80?'A':pct>=70?'B+':pct>=60?'B':pct>=50?'C':pct>=35?'D':'F';
  const pass      = pct>=35;
  const gradeColor= pct>=60?'#16A34A':pct>=35?'#E8622A':'#DC2626';
  ...
  return `
  <div class="rc-page" style="${isPageInBatch ? 'page-break-after: always; break-after: page; margin-bottom: 24px;' : ''}">
    <div class="hdr">
    ...
    <div class="result-box" style="border:2px solid ${pass?'#16A34A':'#DC2626'};background:${pass?'#F0FDF4':'#FEF2F2'}">
      <div class="grade" style="color:${gradeColor}">${grade}</div>
      <div class="pct" style="color:${gradeColor}">${pct}%</div>
      <div class="status" style="color:${pass?'#16A34A':'#DC2626'}">${pass?'✅ PASS':'❌ FAIL'}</div>
    </div>
    <div class="sign-row">
...
function _wrapReportCardHTML(bodyHtml, title = 'Performance Report') {
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .result-box{text-align:center;padding:20px;border-radius:12px;margin-bottom:18px}
    .grade{font-size:56px;font-weight:900;line-height:1}
    .pct{font-size:24px;font-weight:800;margin:4px 0}
...
async function downloadAllReportCards(examId){
  ...
  showLoader('Exporting Reports', `Generating reports for ${stuList.length} students…`, '📊');

  try {
    const examCleanName = (e.name || 'Exam').replace(/[^a-zA-Z0-9_-]/g, '_');
    const examDate = (e.date || '').replace(/[^a-zA-Z0-9_-]/g, '_');
    const subfolder = `Reports/Exam_${examCleanName}_Class${e.cls}_${examDate}`;
    ...
    for (let i = 0; i < stuList.length; i++) {
      ...
      // In Electron: silently save individual student PDF
      if (typeof window.classcore !== 'undefined' && window.classcore.savePDFSilent) {
        const studentCleanName = (s.name || 'Student').replace(/[^a-zA-Z0-9_-]/g, '_');
        const filename = `ReportCard_${studentCleanName}_${examCleanName}`;
        const singleBody = _buildReportCardBody(e, s, subs, false);
        const singleHtml = _wrapReportCardHTML(singleBody, filename);
        const res = await window.classcore.savePDFSilent(singleHtml, filename, 'A4', subfolder);
        if (res && res.folderPath) targetFolder = res.folderPath;
      }

      // Add to combined multi-page document
      combinedPagesHtml += _buildReportCardBody(e, s, subs, true);
      exportedCount++;
    }

    if (exportedCount === 0) {
      hideLoader();
      toast('No marked students found', 'warn');
      return;
    }

    // In Electron: also save combined master document and open the folder ONCE
    if (typeof window.classcore !== 'undefined' && window.classcore.savePDFSilent) {
      const combinedFilename = `All_ReportCards_${examCleanName}_Class${e.cls}`;
      const combinedHtml = _wrapReportCardHTML(combinedPagesHtml, combinedFilename);
      const res = await window.classcore.savePDFSilent(combinedHtml, combinedFilename, 'A4', subfolder);
      hideLoader();
      toast(`✅ Exported ${exportedCount} student reports to Reports folder!`, 'success');
      const folderToOpen = (res && res.folderPath) ? res.folderPath : targetFolder;
      if (folderToOpen && window.classcore.openPath) {
        window.classcore.openPath(folderToOpen);
      }
    } else {
      // In web browser: download single combined multi-page document once
      hideLoader();
      const combinedFilename = `All_ReportCards_${examCleanName}_Class${e.cls}`;
      const combinedHtml = _wrapReportCardHTML(combinedPagesHtml, combinedFilename);
      await generatePDF(combinedHtml, combinedFilename, 'A4', true);
      toast(`✅ Exported report cards for ${exportedCount} students!`, 'success');
    }
  } catch (err) {
    hideLoader();
