...
contextBridge.exposeInMainWorld('classcore', {
  ...
  onUpdateRetrying: (cb) => {
    ipcRenderer.removeAllListeners('update-retrying');
    ipcRenderer.on('update-retrying', (_event, data) => cb(data));
  },

  onUpdateManualFallback: (cb) => {
    ipcRenderer.removeAllListeners('update-manual-fallback');
    ipcRenderer.on('update-manual-fallback', (_event, data) => cb(data));
  },
